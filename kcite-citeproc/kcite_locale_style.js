...
// Function to load CSL style from external file (server-side only)
function loadStyleFromFile(filename) {
  try {
    if (typeof require !== "undefined") {
      ...
      console.warn("File system access not available - using fallback style");
    }
  } catch (error) {
    console.error("Error loading CSL file:", filename, error);
  }
  return null;
}

// apa
kcite_styles["apa"] = loadStyleFromFile("apa.csl");

// chicago_author_date
kcite_styles["chicago-author-date"] = loadStyleFromFile(
  "chicago-author-date.csl"
);

// harvard-cite-them-right
kcite_styles["harvard-cite-them-right"] = loadStyleFromFile(
  "harvard-cite-them-right.csl"
);

// ieee
kcite_styles["ieee"] = loadStyleFromFile("ieee.csl");

// modern-language-association
kcite_styles["modern-language-association"] = loadStyleFromFile(
  "modern-language-association.csl"
);

// nature
kcite_styles["nature"] = loadStyleFromFile("nature.csl");

// vancouver
kcite_styles["vancouver"] = loadStyleFromFile("vancouver.csl");

// american-chemical-society
kcite_styles["american-chemical-society"] = loadStyleFromFile(
