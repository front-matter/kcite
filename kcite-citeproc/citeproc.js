...
var CSL = {
  STATUTE_SUBDIV_GROUPED_REGEX:
    /((?:^| )(?:art|ch|Ch|subch|p|pp|para|subpara|pt|r|sec|subsec|Sec|sch|tit)\.)/g,
  STATUTE_SUBDIV_PLAIN_REGEX:
    /(?:(?:^| )(?:art|ch|Ch|subch|p|pp|para|subpara|pt|r|sec|subsec|Sec|sch|tit)\.)/,
  ...
  TAG_ESCAPE: function (str) {
    ...
    mx = str.match(
      /(\"|\'|<span\s+class=\"no(?:case|decor)\">.*?<\/span>|<\/?(?:i|sc|b)>|<\/span>)/g
    );
    lst = str.split(
      /(?:\"|\'|<span\s+class=\"no(?:case|decor)\">.*?<\/span>|<\/?(?:i|sc|b)>|<\/span>)/g
    );
...
CSL.getSortCompare = function () {
  ...
  try {
    ...
    var collationFactory = Components.classes[
      "@mozilla.org/intl/collation-factory;1"
    ].getService(Components.interfaces.nsICollationFactory);
    var collation = collationFactory.CreateCollation(
...
CSL.Output.Queue.prototype.openLevel = function (token, ephemeral) {
  ...
  if (this.nestedBraces) {
    blob.strings.prefix = blob.strings.prefix.replace(
      this.nestedBraces[0][0],
      this.nestedBraces[0][1]
    );
    blob.strings.prefix = blob.strings.prefix.replace(
      this.nestedBraces[1][0],
      this.nestedBraces[1][1]
    );
    blob.strings.suffix = blob.strings.suffix.replace(
      this.nestedBraces[0][0],
      this.nestedBraces[0][1]
    );
    blob.strings.suffix = blob.strings.suffix.replace(
  ...
  for (i = 0, ilen = blobs.length; i < ilen; i += 1) {
    ...
    if ("string" === typeof blobjr.blobs) {
      if ("number" === typeof blobjr.num) {
        ...
        if (b && b.length) {
          b =
            txt_esc(blobjr.strings.prefix, state.tmp.nestedBraces) +
            b +
            txt_esc(blobjr.strings.suffix, state.tmp.nestedBraces);
          if (
            ...
            for (j = 0, jlen = blobjr.decorations.length; j < jlen; j += 1) {
              params = blobjr.decorations[j];
              if (params[0] === "@showid") {
                b = state.fun.decorate[params[0]][params[1]](
                  state,
                  b,
                  params[2]
                );
  ...
  if (
    ...
    if (!state.tmp.suppress_decorations) {
      for (i = 0, ilen = blob.decorations.length; i < ilen; i += 1) {
        ...
        blobs_start = state.fun.decorate[params[0]][params[1]](
          state,
          blobs_start,
          params[2]
        );
    ...
    if (b && b.length) {
      ...
      b =
        txt_esc(use_prefix, state.tmp.nestedBraces) +
        b +
        txt_esc(use_suffix, state.tmp.nestedBraces);
      if (state.tmp.count_offset_characters) {
    ...
    if (!state.tmp.suppress_decorations) {
      for (i = 0, ilen = blob.decorations.length; i < ilen; i += 1) {
        params = blob.decorations[i];
        if (
          ["@bibliography", "@display", "@showid"].indexOf(params[0]) === -1
        ) {
          continue;
        }
...
CSL.Output.Queue.purgeNearsidePrefixChars = function (myblob, chr) {
  ...
  if ("object" === typeof myblob) {
    if (
      CSL.TERMINAL_PUNCTUATION.indexOf(chr) > -1 &&
      CSL.TERMINAL_PUNCTUATION.slice(0, -1).indexOf(
        myblob.strings.prefix.slice(0, 1)
      ) > -1
    ) {
...
CSL.Output.Queue.purgeNearsideSuffixSpaces = function (myblob, chr) {
  if ("object" === typeof myblob) {
    if (" " === chr && " " === myblob.strings.suffix.slice(-1)) {
      ...
      chr = CSL.Output.Queue.purgeNearsideSuffixSpaces(
        myblob.blobs[myblob.blobs.length - 1],
        chr
      );
    } else {
      chr = myblob.strings.suffix.slice(-1);
  ...
  if ("string" === typeof myblobs) {
    ...
    if (dpref) {
      for (j = 0, jlen = myblobs.length - 1; j < jlen; j += 1) {
        ...
        if (TERMS.indexOf(t) === -1 || TERMS.indexOf(dpref) === -1) {
    ...
    if (suffix === " ") {
      CSL.Output.Queue.purgeNearsideSuffixSpaces(
        myblobs[myblobs.length - 1],
        " "
      );
    }
    var lst = [];
    ...
    for (i = 0, ilen = myblobs.length; i < ilen; i += 1) {
          ...
          if (decorations[j][0] === "@quotes" && decorations[j][1] === "true") {
            ...
            if (swapchar) {
              if ("string" === typeof doblob.blobs) {
                if (
                  SWAPS.indexOf(doblob.blobs.slice(-1)) === -1 ||
                ...
                if (
                  SWAPS.indexOf(
                    doblob.blobs.slice(-1)[0].strings.suffix.slice(-1)
                  ) === -1 ||
                  (TERMS.indexOf(
                    doblob.blobs.slice(-1)[0].strings.suffix.slice(-1)
                  ) > -1 &&
      ...
      if (doblob.strings.delimiter && doblob.blobs.length > 1) {
      ...
      if (doblob.strings.delimiter) {
        if (
          doblob.strings.delimiter.slice(-1) === " " &&
          "object" === typeof doblob.blobs &&
...
CSL.expandMacro = function (macro_key_token) {
  ...
  CSL.Node.group.build.call(
    macro_key_token,
    this,
    this[this.build.area].tokens,
    true
  );
...
CSL.DateParser = function () {
  ...
  rex =
    ...
    ")";
  rexdash = new RegExp(
    rex.replace(/%%NUMD%%/g, "-").replace(/%%DATED%%/g, "-")
  );
  rexdashslash = new RegExp(
    rex.replace(/%%NUMD%%/g, "-").replace(/%%DATED%%/g, "/")
  );
  rexslashdash = new RegExp(
    rex.replace(/%%NUMD%%/g, "/").replace(/%%DATED%%/g, "-")
  );
  seasonstrs = [];
  seasonrexes = [];
      ...
      for (j = 0, jlen = this.mabbrevs.length; j < jlen; j += 1) {
        ...
        if (j === i) {
          for (k = 0, klen = this.mabbrevs[i].length; k < klen; k += 1) {
            if (
              this.mabbrevs[i][k] ===
              lst[i].slice(0, this.mabbrevs[i][k].length)
            ) {
              skip = true;
              break;
            }
          ...
          for (k = 0, klen = this.mabbrevs[j].length; k < klen; k += 1) {
            ...
            if (this.mabbrevs[j][k] === lst[i].slice(0, abbrevlen)) {
              while (
                this.msets[j][k].slice(0, abbrevlen) ===
                lst[i].slice(0, abbrevlen)
              ) {
                if (
                  abbrevlen > lst[i].length ||
                  abbrevlen > this.msets[j][k].length
                ) {
                  CSL.debug(
          ...
          if (extend.hasOwnProperty(jkey)) {
            for (kkey in extend[jkey]) {
              if (extend[jkey].hasOwnProperty(kkey)) {
                ...
                this.mabbrevs[jkey][kkey] = this.msets[jkey][kkey].slice(
                  0,
                  abbrevlen
                );
              }
...
CSL.Engine = function (sys, style, lang, forceLang) {
  ...
  if (this.opt.development_extensions.csl_reverse_lookup_support) {
    ...
    this.setCslNodeIds = function (myxml, nodename) {
      ...
      for (
        var i = 0, ilen = this.sys.xml.numberofnodes(children);
        i < ilen;
        i += 1
      ) {
...
CSL.Engine.prototype.getTerm = function (term, form, plural, gender, mode) {
  ...
  var ret = CSL.Engine.getField(
    CSL.LOOSE,
    this.locale[this.opt.lang].terms,
    term,
    form,
    plural,
    gender
...
CSL.Engine.prototype.setOpt = function (token, name, value) {
  if (token.name === "style" || token.name === "cslstyle") {
    ...
    ["name-form", "name-delimiter", "names-delimiter"].indexOf(name) === -1
  ) {
...
CSL.Engine.prototype.fixOpt = function (token, name, localname) {
  ...
  if ("name" === token.name || "names" === token.name) {
    if (
      "undefined" === typeof token.strings[localname] &&
      "undefined" !== typeof this[this.build.root].opt[name]
    ) {
      ...
      if (value) {
        ...
        if (item.locator) {
          ...
          if (
            ...
            labelstr =
              " " + CSL.STATUTE_SUBDIV_STRINGS_REVERSE[later_label] + " ";
...
CSL.getBibliographyEntries = function (bibsection) {
  ...
  for (pos = 0; pos < len; pos += 1) {
    ...
    bib_entry.decorations = [["@bibliography", "entry"]].concat(
      this[this.build.area].opt.layout_decorations
    );
      ...
      for (ppos = llen; ppos > -1; ppos += -1) {
        if (topblobs[ppos].blobs && topblobs[ppos].blobs.length !== 0) {
          ...
          if (chr && topblobs[ppos].strings.suffix.slice(-1) === chr) {
            topblobs[ppos].strings.suffix = topblobs[ppos].strings.suffix.slice(
              0,
              -1
            );
          }
    ...
    if (this.opt.development_extensions.locator_date_and_revision) {
      if (item.locator) {
        ...
        if (idx > -1) {
          ...
          item["locator-revision"] = raw_locator
            .replace(/^\s+/, "")
            .replace(/\s+$/, "");
        }
      }
  ...
  if (this.opt.update_mode === CSL.POSITION) {
    for (i = 0; i < 2; i += 1) {
      ...
      for (j = 0, jlen = citations.length; j < jlen; j += 1) {
        ...
        for (
          var k = 0, klen = onecitation.sortedItems.length;
          k < klen;
          k += 1
        ) {
          if (
...
CSL.getCitationCluster = function (inputList, citationID) {
  ...
  if (citationID) {
    this.registry.citationreg.citationById[
      citationID
    ].properties.backref_index = false;
    this.registry.citationreg.citationById[
      citationID
    ].properties.backref_citation = false;
  }
  ...
  if (
    CSL.TERMINAL_PUNCTUATION.slice(0, -1).indexOf(delimiter.slice(0, 1)) > -1
  ) {
  ...
  for (pos = 0, len = myblobs.length; pos < len; pos += 1) {
    ...
    if (objects.length && "string" === typeof composite[0]) {
      ...
      if (tmpstr && tmpstr.slice(0, 1) === ",") {
        ...
        "string" == typeof objects.slice(-1)[0] &&
        objects.slice(-1)[0].slice(-1) === ","
      ) {
        objects.push(" " + tmpstr);
      } else if (tmpstr) {
  ...
  if (result) {
    if (
      CSL.TERMINAL_PUNCTUATION.indexOf(this.tmp.last_chr) > -1 &&
    ...
    result =
      txt_esc(this.citation.opt.layout_prefix) +
      result +
      txt_esc(use_layout_suffix);
...
CSL.citeStart = function (Item, item) {
  ...
  if (
    this.opt.development_extensions.flip_parentheses_to_braces &&
    item &&
    item.prefix
  ) {
...
CSL.citeEnd = function (Item, item) {
  if (this.tmp.disambig_restore) {
    this.registry.registry[Item.id].disambig.names =
      this.tmp.disambig_restore.names;
    this.registry.registry[Item.id].disambig.givens =
      this.tmp.disambig_restore.givens;
  }
  ...
  if (this.tmp.original_date && this.tmp.renders_collection_number) {
    var buf = [];
    for (
      var i = this.tmp.original_date.list.length - 1;
      i > this.tmp.original_date.pos;
      i += -1
    ) {
...
CSL.localeResolve = function (langstr) {
  ...
  if ("undefined" === typeof ret.base) {
    CSL.debug(
      "Warning: unknown locale " + langstr + ", setting fallback to en-US"
    );
...
CSL.Engine.prototype.localeSet = function (myxml, lang_in, lang_out) {
  ...
  if (this.sys.xml.nodeNameIs(myxml, "locale")) {
    ...
    for (
      pos = 0, len = this.sys.xml.numberofnodes(nodes);
      pos < len;
      pos += 1
    ) {
      blob = nodes[pos];
  ...
  for (pos = 0, len = this.sys.xml.numberofnodes(nodes); pos < len; pos += 1) {
    ...
    if (this.sys.xml.getAttributeValue(term, "gender")) {
      this.opt["noun-genders"][termname] = this.sys.xml.getAttributeValue(
        term,
        "gender"
      );
    }
    ...
    if (
      this.sys.xml.numberofnodes(this.sys.xml.getNodesByName(term, "multiple"))
    ) {
  ...
  for (pos = 0, len = this.sys.xml.numberofnodes(nodes); pos < len; pos += 1) {
    if (true) {
      date = nodes[pos];
      this.locale[lang_out].dates[
        this.sys.xml.getAttributeValue(date, "form")
      ] = date;
    }
  }
};
  ...
  build: function (state, target) {
    ...
    if (this.tokentype === CSL.START || this.tokentype === CSL.SINGLETON) {
      ...
      if (state.build.extension) {
        ...
        func = function (state, Item, item) {
          ...
          if (
            ...
            !(state.tmp.just_looking && this.variables[0] !== "issued")
          ) {
      ...
      func = function (state, Item) {
        ...
        if (
          this.variables[0] === "original-date" &&
  ...
  build: function (state, target) {
    ...
    func = function (state, Item) {
      ...
      if ("undefined" !== typeof value) {
        ...
        if (this.strings.form) {
          value = CSL.Util.Dates[this.strings.name][this.strings.form](
            state,
            value,
            gender
          );
          if ("month" === this.strings.name) {
            ...
            if (value_end) {
              value_end = CSL.Util.Dates[this.strings.name][this.strings.form](
                state,
                value_end,
                gender
              );
      ...
      func = function (state, Item) {
        ...
        if (state.tmp.group_context.mystack.length) {
          state.output.current.value().parent =
            state.tmp.group_context.value()[4];
        }
        ...
        state.tmp.group_context.push(
          [
            ...
            label_form,
            this.strings.set_parallel_condition,
          ],
          CSL.LITERAL
        );
      ...
      func = function (state, Item) {
        ...
        if (flag[2] || (flag[0] && !flag[1])) {
          ...
          if (state.tmp.group_context.value()[3]) {
            state.output.current.mystack[
              state.output.current.mystack.length - 2
            ].strings.delimiter = state.tmp.group_context.value()[3];
          }
        }
      };
      this.execs.push(func);
...
CSL.Node.institution = {
  build: function (state, target) {
    if ([CSL.SINGLETON, CSL.START].indexOf(this.tokentype) > -1) {
      if (
        "string" === typeof state.build.name_delimiter &&
  ...
  build: function (state, target) {
      ...
      if (state.build.area === "citation") {
        ...
        func = function (state, Item, item) {
          ...
          if (item && item.prefix) {
            ...
            if (
              CSL.TERMINAL_PUNCTUATION.slice(0, -1).indexOf(prefix.slice(-1)) >
                -1 &&
    ...
    if (this.tokentype === CSL.START) {
      ...
      if (this.locale_raw) {
        ...
        state.tmp.cite_affixes[my_tok.locale].delimiter =
          this.strings.delimiter;
...
CSL.NameOutput.prototype._truncateNameList = function (
  ...
  if (
    this.state.opt.max_number_of_names &&
    lst.length > 50 &&
...
CSL.NameOutput.prototype._checkNickname = function (name) {
  if (["interview", "personal_communication"].indexOf(this.Item.type) > -1) {
    ...
    if (
      ...
      var myLocalName =
        this.state.transform.abbrevs["default"].nickname[author];
      if (myLocalName) {
...
CSL.NameOutput.prototype._joinAnd = function (blobs, tokenname) {
  return this._join(
    blobs,
    this[tokenname].strings.delimiter,
    this[tokenname].and.single,
    this[tokenname].and.multiple,
    tokenname
  );
};
CSL.NameOutput.prototype._join = function (
  blobs,
  delimiter,
  single,
  multiple,
    ...
    if (this.state.tmp.disambig_request) {
      ...
      if (this.state.opt["disambiguate-add-givenname"]) {
        param = this.state.registry.namereg.evalname(
          ...
          i,
          param,
          this.name.strings.form,
          this.name.strings["initialize-with"]
        );
      }
    } else {
      param = paramx;
...
CSL.NameOutput.prototype.setCommonTerm = function () {
  ...
  if (this.label) {
    if (this.label.before) {
      has_term = this.state.getTerm(
        this.common_term,
        this.label.before.strings.form,
        0
      );
    } else if (this.label.after) {
      has_term = this.state.getTerm(
        this.common_term,
        this.label.after.strings.form,
        0
      );
    }
  }
  ...
  for (var i = 0, ilen = this.variables.length - 1; i < ilen; i += 1) {
    ...
    if (this.freeters[v].length || this.freeters[vv].length) {
      if (
        this.etal_spec[this.variable_offset[v]] !==
          this.etal_spec[this.variable_offset[vv]] ||
    ...
    for (var j = 0, jlen = this.persons[v].length; j < jlen; j += 1) {
      if (
        this.etal_spec[this.variable_offset[v] + freeters_offset + j + 1] !==
          this.etal_spec[this.variable_offset + freeters_offset + j + 1] ||
      ...
      switch (this.institution.strings["institution-parts"]) {
        case "short":
          if (primary["short"].length) {
            ...
            institution = [
              this._renderOneInstitutionPart(primary["short"], short_style),
            ];
          } else {
            long_style = this._getLongStyle(primary, v, j);
            institution = [
              this._renderOneInstitutionPart(primary["long"], long_style),
            ];
          }
          break;
        case "short-long":
...
CSL.NameOutput.prototype._renderOnePersonalName = function (value, pos, i) {
  ...
  if (!romanesque) {
    ...
    this.name.strings["name-as-sort-order"] === "all" ||
    (this.name.strings["name-as-sort-order"] === "first" && i === 0)
  ) {
    ...
    if (
      ...
      if (
        ["'", "\u02bc", "\u2019"].indexOf(name["dropping-particle"].slice(-1)) >
        -1
      ) {
...
CSL.NameOutput.prototype._parseName = function (name) {
  ...
  if (!name["non-dropping-particle"] && name.family && !noparse) {
    m = name.family.match(
      /^((?:[a-z][ \'\u2019a-z]*[\s+|\'\u2019]|[DVL][^ ]\s+[a-z]*\s*|[DVL][^ ][^ ]\s+[a-z]*\s*))/
    );
    if (m) {
...
CSL.NameOutput.prototype.getName = function (
      ...
      for (i = 0, ilen = langTags.length; i < ilen; i += 1) {
        ...
        if (name.multi._key[langTag]) {
          ...
          if (!this.state.opt["locale-use-original-name-format"] && false) {
            ...
            if (
              (name.family.replace('"', "", "g") + name.given).match(
                CSL.ROMANESQUE_REGEXP
              )
            ) {
  ...
  if (static_ordering_freshcheck && !this.getStaticOrder(name, true)) {
...
CSL.NameOutput.prototype._splitInstitution = function (value, v, i) {
  ...
  if (
    ...
    for (var j = splitInstitution.length; j > 0; j += -1) {
      ...
      if (
        this.state.transform.abbrevs[jurisdiction]["institution-entire"][str]
      ) {
        var splitLst =
          this.state.transform.abbrevs[jurisdiction]["institution-entire"][str];
...
CSL.dateAsSortKey = function (state, Item, isMacro) {
  ...
  for (i = 0, ilen = CSL.DATE_PARTS_INTERNAL.length; i < ilen; i += 1) {
    ...
    if (elem.slice(0, 4) === "year") {
      ...
      state.output.append(
        CSL.Util.Dates[e]["numeric-leading-zeros"](state, value),
        macroFlag
      );
    }
  }
...
CSL.Node.name = {
  build: function (state, target) {
    ...
    if ([CSL.SINGLETON, CSL.START].indexOf(this.tokentype) > -1) {
      ...
      if (
        this.strings["et-al-subsequent-min"] &&
        this.strings["et-al-subsequent-min"] !== this.strings["et-al-min"]
      ) {
        state.opt.update_mode = CSL.POSITION;
      }
      if (
        this.strings["et-al-subsequent-use-first"] &&
        this.strings["et-al-subsequent-use-first"] !==
          this.strings["et-al-use-first"]
      ) {
        state.opt.update_mode = CSL.POSITION;
      }
      state.build.etal_term = "et-al";
      state.build.name_delimiter = this.strings.delimiter;
      state.build["delimiter-precedes-et-al"] =
...
CSL.Node.number = {
  build: function (state, target) {
    ...
    func = function (state, Item, item) {
      ...
      var rex = new RegExp(
        "(?:&|, | and |" + state.getTerm("page-range-delimiter") + ")"
      );
      ...
      if (this.text_case_normal) {
      ...
      } else if (varname === "locator" && item.locator) {
        item.locator = item.locator.replace(
          /([^\\])\s*-\s*/,
          "$1" + state.getTerm("page-range-delimiter")
        );
        ...
        if (m) {
          ...
          if (
            ...
            state.tmp.group_context.value()[5]
          ) {
        ...
        var node = this;
        if (
          !state.tmp.shadow_numbers[varname] ||
        ...
        var rangeType = "page";
        if (
          ["bill", "gazette", "legislation", "legal_case"].indexOf(Item.type) >
            -1 &&
            ...
            if (firstword) {
              ...
              if (m) {
                ...
                for (var i = 1, ilen = lst.length; i < ilen; i += 1) {
                  newlst.push(
                    state.getTerm(
                      CSL.STATUTE_SUBDIV_STRINGS[m[i - 1].replace(/^\s+/, "")],
                      this.strings.label_form_override
                    )
                  );
                  newlst.push(lst[i].replace(/^\s+/, ""));
            ...
            if (!state.tmp.just_looking) {
              if (item && item["author-only"]) {
                ...
                term = CSL.Output.Formatters["capitalize-first"](
                  state,
                  state.getTerm("reference", "long", "singular")
                );
            ...
            if (CSL.CITE_FIELDS.indexOf(this.variables_real[0]) > -1) {
              ...
              func = function (state, Item) {
                ...
                if (state.transform.abbrevs[hereinafter_info[0]]) {
                  var value =
                    state.transform.abbrevs[hereinafter_info[0]].hereinafter[
                      hereinafter_info[1]
                    ];
                  if (value) {
                    state.tmp.group_context.value()[2] = true;
    ...
    if (nameList && nameList.length) {
      ...
      for (var i = 0, ilen = nameList.length; i < ilen; i += 1) {
        if (
          ...
          (nameList[i].literal || (nameList[i].family && !nameList[i].given))
        ) {
          institutions += 1;
...
CSL.Attributes["@variable"] = function (state, arg) {
  ...
  if ("label" === this.name && this.variables[0]) {
    ...
    func = function (state, Item, item) {
      ...
      for (pos = 0; pos < len; pos += 1) {
        if (
          state.tmp.done_vars.indexOf(variables[pos]) === -1 &&
          !(
            item &&
            Item.type === "legal_case" &&
            item["suppress-author"] &&
        ...
        if ("hereinafter" === variables[pos] && state.sys.getAbbreviation) {
          var hereinafter_info = state.transform.getHereinafter(Item);
          state.transform.loadAbbreviation(
            hereinafter_info[0],
            "hereinafter",
            hereinafter_info[1]
          );
...
CSL.Attributes["@is-numeric"] = function (state, arg) {
  ...
  func = function (state, Item, item) {
    ...
    for (pos = 0; pos < len; pos += 1) {
      ...
      if (
        !state.tmp.shadow_numbers[variables[pos]].numeric &&
        !(
          variables[pos] === "title" &&
          Item[variables[pos]] &&
          Item[variables[pos]].slice(-1) ===
            "" + parseInt(Item[variables[pos]].slice(-1))
        )
      ) {
        numeric = false;
        break;
...
CSL.Attributes["@disambiguate"] = function (state, arg) {
  if (
    ...
    ["if", "else-if"].indexOf(this.name) > -1
  ) {
    if (arg === "true") {
      ...
      if (arg === "title") {
        ...
        if (Item.jurisdiction) {
          ...
          if (!m) {
            ...
            this.strings["text-case"] = "passthrough";
            for (
              var i = 0, ilen = state.opt.english_locale_escapes.length;
              i < ilen;
              i += 1
            ) {
...
CSL.Transform = function (state) {
  ...
  function abbreviate(
    ...
    if (
      ["publisher-place", "event-place", "jurisdiction"].indexOf(
        myabbrev_family
      ) > -1
    ) {
    ...
    if (state.sys.getAbbreviation) {
      ...
      if (
        ...
        if (state.transform.abbrevs[jurisdiction][myabbrev_family][basevalue]) {
          value =
            state.transform.abbrevs[jurisdiction][myabbrev_family][basevalue];
        }
      }
    }
        ...
        if (this.abbrevs[tryList[i]][category][orig]) {
          if (i < tryList.length) {
            this.abbrevs[jurisdiction][category][orig] =
              this.abbrevs[tryList[i]][category][orig];
          }
          break;
        }
      }
  ...
  function getHereinafter(Item) {
    ...
    var hereinafter_key =
      hereinafter_author_title.join(", ") + hereinafter_metadata;
  ...
  if (this.use_parallels) {
    ...
    if (this.sets.value().length === 0) {
      ...
      for (pos = 0, len = this.cite.back.length; pos < len; pos += 1) {
        ...
        if (
          x === "original-date" &&
          this.cite["original-date"] &&
          this.cite["original-date"].value
        ) {
          has_date = true;
          break;
        }
...
CSL.Util.fixDateNode = function (parent, pos, node) {
    ...
    if ("date-part" === this.sys.xml.nodename(subnode)) {
      ...
      for (attr in subchildren) {
        if (subchildren.hasOwnProperty(attr)) {
          ...
          this.sys.xml.setAttributeOnNodeIdentifiedByNameAttribute(
            datexml,
            "date-part",
            partname,
            attr,
  ...
  if ("year" === this.sys.xml.getAttributeValue(node, "date-parts")) {
    ...
    "year-month" === this.sys.xml.getAttributeValue(node, "date-parts")
  ) {
    this.sys.xml.deleteNodeByNameAttribute(datexml, "day");
  }
...
CSL.Util.Names.unInitialize = function (state, name) {
  ...
  for (i = 0, ilen = namelist.length; i < ilen; i += 1) {
    if (
      CSL.ALL_ROMANESQUE_REGEXP.exec(namelist[i].slice(0, -1)) &&
      namelist[i] &&
      namelist[i] !== namelist[i].toUpperCase()
    ) {
      namelist[i] =
        namelist[i].slice(0, 1) +
        namelist[i].slice(1, 2).toLowerCase() +
        namelist[i].slice(2);
    }
    ret += namelist[i];
    if (i < ilen - 1) {
  ...
  for (i = 0, ilen = namelist.length; i < ilen; i += 2) {
    if (isAbbrev[i]) {
      if (i < namelist.length - 2) {
        ...
        if (
          (!terminator ||
            (terminator.slice(-1) && terminator.slice(-1) !== " ")) &&
          namelist[i].length &&
...
CSL.Util.substituteEnd = function (state, target) {
  ...
  toplevel =
    "names" === this.name && state.build.substitute_level.value() === 0;
  hasval =
    "string" ===
    typeof state[state.build.area].opt["subsequent-author-substitute"];
  var subrule =
    state[state.build.area].opt["subsequent-author-substitute-rule"];
  if (toplevel && hasval) {
      ...
      if (printing && state.tmp.area === "bibliography") {
        if (!state.tmp.rendered_name) {
          if ("partial-each" === subrule || "partial-first" === subrule) {
            ...
            state.tmp.rendered_name = state.output.string(
              state,
...
CSL.Engine.prototype.processNumber = function (
  node,
  ItemObject,
  variable,
  type
  ...
  if (this.tmp.shadow_numbers[variable]) {
    if (this.tmp.shadow_numbers[variable].numeric) {
      for (
        var i = 0, ilen = this.tmp.shadow_numbers[variable].values.length;
        i < ilen;
        i += 2
      ) {
  ...
  if ("undefined" !== typeof num) {
    ...
    var rangeType = "page";
    if (
      ["bill", "gazette", "legislation", "legal_case"].indexOf(type) > -1 &&
    ...
    if (["page", "page-first"].indexOf(variable) > -1) {
      ...
      if (m) {
        this.tmp.shadow_numbers[variable].label =
          CSL.STATUTE_SUBDIV_STRINGS[m[0]];
        var mm = num.match(/[^ ]+\s+(.*)/);
        num = mm[1];
      }
    }
      ...
      if (odd) {
        if (elements[i]) {
          if (elements[i].match(/[0-9]/)) {
            if (
              elements[i - 1] &&
              elements[i - 1].match(/^\s*\\*[\-\u2013]+\s*$/)
            ) {
              var middle = this.tmp.shadow_numbers[variable].values.slice(-1);
              if (middle[0][1].indexOf("\\") == -1) {
                if (
                  elements[i - 2] &&
                  ("" + elements[i - 2]).match(/[a-zA-Z]*[0-9]+$/) &&
                  elements[i].match(/^[a-zA-Z]*[0-9]+/)
                ) {
                  var start =
                    this.tmp.shadow_numbers[variable].values.slice(-2);
                  middle[0][1] = this.getTerm(rangeType + "-range-delimiter");
                  if (this.opt[rangeType + "-range-format"]) {
                    var newstr = this.fun[rangeType + "_mangler"](
                      start[0][1] + "-" + elements[i]
                    );
                    newstr = newstr.split(
                      this.getTerm(rangeType + "-range-delimiter")
                    );
                    elements[i] = newstr[1];
                  }
                  count = count + 1;
          ...
          if (i === elements.length - 1) {
            if (elements.length > 1 || subelements.length > 1) {
              ...
              if (
                matchterm &&
                !subelements[subelements.length - 1].match(/[0-9]/)
              ) {
                matchterm = matchterm
                  .replace(".", "")
                  .toLowerCase()
                  .split(/\s+/)[0];
                if (
                  subelements[subelements.length - 1]
                    .slice(0, matchterm.length)
                    .toLowerCase() === matchterm
                ) {
          ...
          if (elements[i].match(/^[1-9][0-9]*$/)) {
            ...
            this.tmp.shadow_numbers[variable].values.push([
              "NumericBlob",
              elements[i],
              node,
            ]);
...
CSL.Util.FlipFlopper = function (state) {
  ...
  makeHashes = function (tagdefs) {
    ...
    for (pos = 0; pos < len; pos += 1) {
      ...
      okReverse[tagdefs[pos][3]] = [
        tagdefs[pos][3],
        [tagdefs[pos][4][2], tagdefs[pos][1]],
      ];
    }
...
CSL.Util.FlipFlopper.prototype.getSplitStrings = function (str) {
  ...
  for (pos = len; pos > 0; pos += -2) {
    if (strs[pos - 1].slice(strs[pos - 1].length - 1) === "\\") {
      newstr =
        strs[pos - 1].slice(0, strs[pos - 1].length - 1) +
        strs[pos] +
        strs[pos + 1];
  ...
  for (posA = 1; posA < lenA; posA += 2) {
    ...
    if (this.closeTagsHash[tag]) {
      ...
      if (
        !sameAsOpen ||
        (openRev > -1 && (openRev < flipRev || flipRev === -1))
      ) {
        ibeenrunned = false;
  ...
  for (pos = 0; pos < len; pos += 1) {
    ...
    if (
      sep.length &&
      sep[0] !== "<" &&
      this.openToDecorations[sep] &&
      this.quotechars.indexOf(sep.replace(/\s+/g, "")) === -1
    ) {
    ...
    for (posA = 1; posA < lenA; posA += 2) {
      ...
      if (this.openToCloseHash[tag]) {
        ...
        if (tag === '<span class="nodecor">') {
          fulldecor = this.state[this.state.tmp.area].opt.topdecor
            .concat(this.blob.alldecor)
            .concat([[["@quotes", "inner"]]]);
          ...
          for (posB = 0; posB < lenB; posB += 1) {
            ...
            for (posC = 0; posC < lenC; posC += 1) {
              decor = level[posC];
              if (
                ["@font-style", "@font-weight", "@font-variant"].indexOf(
                  decor[0]
                ) > -1
              ) {
                param = this.addFlipFlop(
                  newblobnest,
                  this.okReverseHash[decor[0]]
                );
              }
            }
          }
        }
        expected_rendering.push(
          this.state.fun.decorate[param[0]][param[1]](this.state)
        );
...
CSL.Output.Formatters.sentence = function (state, string) {
  var str = CSL.Output.Formatters.doppelString(string, CSL.TAG_ESCAPE);
  str.string =
    str.string.slice(0, 1).toUpperCase() + str.string.substr(1).toLowerCase();
  return CSL.Output.Formatters.undoppelString(str);
};
CSL.Output.Formatters["capitalize-all"] = function (state, string) {
  ...
  for (pos = 0; pos < len; pos += 1) {
    if (strings[pos].length > 1) {
      strings[pos] =
        strings[pos].slice(0, 1).toUpperCase() +
        strings[pos].substr(1).toLowerCase();
...
CSL.Output.Formatters.title = function (state, string) {
  ...
  words = myret.slice();
  isAllUpperCase =
    str.string.toUpperCase() === string && !str.string.match(/[0-9]/);
  newString = "";
    ...
    if (
      ...
      if (isAllUpperCase || words[pos] === lowerCaseVariant) {
        ...
        for (var i = 0, ilen = SKIP_WORDS.length; i < ilen; i += 1) {
          ...
          if (idx > -1) {
            tmp =
              lowerCaseVariant.slice(0, idx) +
              lowerCaseVariant.slice(idx + skipword.length);
            if (!tmp.match(/[a-zA-Z]/)) {
              skip = true;
            }
          }
        ...
        if (!totallyskip) {
          if (skip && notfirst && notlast && !aftercolon) {
            ...
            words[pos] =
              upperCaseVariant.slice(0, 1) + lowerCaseVariant.substr(1);
          }
        }
      }
    ...
    if (!this.registry[item]) {
      ...
      for (j = 0, jlen = this.generate.rules.length; j < jlen; j += 1) {
        if (Item.type === this.generate.rules[j].from) {
          var needsRule = true;
          for (
            k = 0, klen = this.generate.rules[j].triggers.length;
            k < klen;
            k += 1
          ) {
...
CSL.Registry.NameReg = function (state) {
  ...
  evalname = function (
    ...
    if (param === 0) {
      ...
      pos = this.namereg[pkey].ikey[ikey].skey[skey].items.indexOf(
        "" + item_id
      );
      ...
      if (
        this.namereg[pkey].ikey[ikey].skey[skey].items.indexOf("" + item_id) ===
        -1
      ) {
        ...
        if (this.nameind[id].hasOwnProperty(fullkey)) {
          ...
          if (
            ...
            if (this.namereg[pkey].ikey[ikey].skey[skey].items.length === 1) {
              this.namereg[pkey].ikey[ikey].items.push(
                this.namereg[pkey].ikey[ikey].skey[skey].items[0]
              );
              this.namereg[pkey].ikey[ikey].skey[skey].items = [];
            }
            for (
              ppos = 0,
                llen = this.namereg[pkey].ikey[ikey].skey[skey].items.length;
              ppos < llen;
              ppos += 1
            ) {
          ...
          if (ikey && this.namereg[pkey].ikey[ikey]) {
            ...
            if (this.namereg[pkey].ikey[ikey].items.length === 1) {
              this.namereg[pkey].items.push(
                this.namereg[pkey].ikey[ikey].items[0]
              );
              this.namereg[pkey].ikey[ikey].items = [];
            }
            for (
              ppos = 0, llen = this.namereg[pkey].ikey[ikey].items.length;
              ppos < llen;
              ppos += 1
            ) {
              ret[this.namereg[pkey].ikey[ikey].items[ppos]] = true;
          ...
          if (pkey) {
            ...
            for (
              ppos = 0, llen = this.namereg[pkey].items.length;
              ppos < llen;
              ppos += 1
            ) {
  ...
  addname = function (item_id, nameobj, pos) {
    ...
    nameobj = res.name;
    if (
      state.opt["givenname-disambiguation-rule"] &&
...
CSL.Disambiguation.prototype.disYears = function () {
  ...
  if (this.clashes[1]) {
    for (
      pos = 0, len = this.lists[this.listpos][1].length;
      pos < len;
      pos += 1
    ) {
  ...
  if ("disNames" === this.modes[this.modeindex]) {
    ...
    if (
      ...
      ("number" !== typeof this.namesMax ||
        this.base.names[this.gnameset] === this.namesMax) &&
  ...
  if (!this.maxNamesByItemId[Item.id]) {
    ...
    for (var i = 0, ilen = base.names.length; i < ilen; i += 1) {
      if (base.names[i] > this.base.names[i]) {
        this.base.givens[i] = this.base.givens[i].concat(
          this.base.givens[i].slice(this.base.names[i])
        );
