...
// Environment compatibility and dependency loading
(function () {
  // Ensure global scope for Node.js and browser environments
  var global = (function () {
  ...
  // For Node.js environment, load required dependencies
  if (typeof require !== "undefined" && typeof module !== "undefined") {
    try {
      // Load XML DOM parser for Node.js
      if (!global.DOMParser) {
        global.DOMParser = require("xmldom").DOMParser;
      }
    } catch (e) {
      // If xmldom is not available, we'll use the built-in fallback
      console.warn("xmldom not available, using built-in XML parser fallback");
    }
  }
})();
...
var CSL = {
  ...
  getAbbrevsDomain: function (state, country, lang) {
    ...
    if (state.opt.availableAbbrevDomains && country && country !== "default") {
      var globalDomainPreference =
        state.locale[state.opt.lang].opts["jurisdiction-preference"];
      var itemDomainPreference = null;
      if (state.locale[lang]) {
        itemDomainPreference =
          state.locale[lang].opts["jurisdiction-preference"];
      }
      if (itemDomainPreference) {
        for (var j = itemDomainPreference.length - 1; j > -1; j--) {
          if (
            state.opt.availableAbbrevDomains[country].indexOf(
              itemDomainPreference[j]
            ) > -1
          ) {
    ...
    for (var i = 0, ilen = segments.length; i < ilen; i++) {
      ...
      for (var j = 0, jlen = langs.length; j < jlen; j++) {
        ...
        if (vals[title.title]) {
          ...
          if (
            shortTitle &&
            shortTitle.toLowerCase() === vals[title.title].toLowerCase()
          ) {
            vals[title.main] = vals[title.title];
            vals[title.subjoin] = "";
            vals[title.sub] = "";
            ...
            var tail = vals[title.title].slice(
              shortTitle.replace(/[\?\!]+$/, "").length
            );
            var top = vals[title.title]
              .replace(tail.replace(/^[\?\!]+/, ""), "")
              .trim();
          ...
          if (vals[title.subjoin]) {
            if (vals[title.subjoin].match(/([\?\!])/)) {
              ...
              vals[title.main] =
                vals[title.main] + narrowSpace + vals[title.subjoin].trim();
    ...
    if (vals[title.main] && vals[title.sub]) {
      ...
      if (sentenceCase) {
        ...
        for (var i = 0, ilen = splits.length; i < ilen; i += 2) {
          splits[i] = CSL.Output.Formatters["capitalize-first"](
            state,
            splits[i]
          );
        }
        for (var i = 1, ilen = splits.length - 1; i < ilen; i += 2) {
          ...
          if (m) {
            var narrowSpace =
              state.opt["default-locale"][0].slice(0, 2).toLowerCase() === "fr"
                ? "\u202f"
                : "";
  ...
  UPDATE_GROUP_CONTEXT_CONDITION: function (
    state,
    str,
    valueTerm,
    token,
    ...
    if (flags.condition) {
      ...
      if (
        !flags.value_seen &&
        flags.condition.test === "comma-safe-numbers-only"
      ) {
        if (value) {
      ...
      if (
        token &&
        token.decorations.filter((o) => o[0] === "@vertical-align").length > 0
      ) {
        state.tmp.just_did_number = false;
  ...
  EVALUATE_GROUP_CONDITION: function (state, flags) {
    ...
    if (flags.condition.test === "empty-label") {
      ...
      testres =
        !flags.condition.termtxt || flags.condition.termtxt.indexOf("%s") > -1;
    } else if (
      ["comma-safe", "comma-safe-numbers-only"].indexOf(flags.condition.test) >
      -1
    ) {
      ...
      if (flags.condition.termtxt) {
        termStartAlpha = flags.condition.termtxt
          .slice(0, 1)
          .match(CSL.ALL_ROMANESQUE_REGEXP);
      }
      ...
      if (num) {
        if (flags.condition.valueTerm) {
          ...
          ["always", "after-number"].indexOf(
            state.opt.require_comma_on_symbol
          ) > -1
        ) {
          testres = true;
        } else {
  ...
  GET_COURT_CLASS: function (state, Item, sortKey) {
    ...
    if (authority) {
      if (
        this.lang &&
        state.locale[this.lang].opts[classType] &&
        state.locale[this.lang].opts[classType][country] &&
        state.locale[this.lang].opts[classType][country][authority]
      ) {
        cls = state.locale[this.lang].opts[classType][country][authority];
      } else if (
        state.locale[state.opt["default-locale"][0]].opts[classType] &&
        state.locale[state.opt["default-locale"][0]].opts[classType][country] &&
        state.locale[state.opt["default-locale"][0]].opts[classType][country][
          authority
        ]
      ) {
    ...
    for (var i = 0, ilen = jurisdictionList.length; i < ilen; i++) {
      ...
      if (item) {
        if (
          ...
          !item["best-jurisdiction"] &&
          state.juris[jurisdiction].types.locator
        ) {
...
CSL.XmlDOM = function (dataObj) {
  ...
  // Enhanced DOMParser initialization with better environment detection
  if ("undefined" == typeof DOMParser) {
    DOMParser = function () {};
    DOMParser.prototype.parseFromString = function (str, contentType) {
      // Try different XML parsing approaches based on environment
      if ("undefined" != typeof ActiveXObject) {
        // Internet Explorer XML parsing
        var xmldata = new ActiveXObject("MSXML.DomDocument");
        xmldata.async = false;
        xmldata.loadXML(str);
        return xmldata;
      } else if ("undefined" != typeof XMLHttpRequest) {
        // Modern browsers with XMLHttpRequest support
        ...
        // Marknote parser fallback
        var parser = new marknote.Parser();
        return parser.parse(str);
      } else {
        // Last resort: create a minimal mock DOM structure
        console.warn("No XML parser available, creating minimal DOM mock");
        return {
...
CSL.getSortCompare = function (default_locale) {
  ...
  strcmp = function (a, b) {
    return CSL.toLocaleLowerCase
      .call(me, a)
      .localeCompare(
        CSL.toLocaleLowerCase.call(me, b),
        default_locale,
        strcmp_opts
      );
  };
...
CSL.Engine.prototype.retrieveItem = function (id) {
    ...
    if (CSL.DATE_VARIABLES.indexOf(key.replace(/^alt-/, "")) > -1) {
      ...
      if (dateobj) {
        ...
        if (this.opt.development_extensions.raw_date_parsing) {
          if (
            dateobj.raw &&
            (!dateobj["date-parts"] || dateobj["date-parts"].length === 0)
          ) {
  ...
  if (this.opt.development_extensions.main_title_from_short_title) {
    var narrowSpaceLocale =
      this.opt["default-locale"][0].slice(0, 2).toLowerCase() === "fr";
...
CSL.Engine.prototype.processCitationCluster = function (
  ...
  for (var i = 0, ilen = citation.citationItems.length; i < ilen; i += 1) {
    ...
    if (Item.id) {
      this.transform.loadAbbreviation(
        "default",
        "hereinafter",
        Item.id,
        Item.language
    ...
    if (this.opt.development_extensions.strict_inputs) {
      ...
      if (preCitation[1]) {
        if (lastNotePos > preCitation[1]) {
          CSL.debug(
            "Note index sequence is not sane at citationsPre[" + i + "]"
          );
        }
  ...
  if (this.opt.development_extensions.strict_inputs) {
    if (citationById[citation.citationID]) {
      CSL.error(
        "Citation with previously referenced citationID " + citation.citationID
      );
    }
    ...
    if (this.opt.development_extensions.strict_inputs) {
      ...
      if (postCitation[1]) {
        if (lastNotePos > postCitation[1]) {
          CSL.debug(
            "Note index sequence is not sane at postCitation[" + i + "]"
          );
        }
      ...
      for (j = 0, jlen = citations.length; j < jlen; j += 1) {
        ...
        for (k = 0, klen = onecitation.sortedItems.length; k < klen; k += 1) {
          if (
            ...
            onecitation.sortedItems[k][1].parallel !== "first"
          ) {
            continue;
          }
        ...
        for (k = 0, klen = citations[j].sortedItems.length; k < klen; k += 1) {
          ...
          var first_id = item[0].legislation_id
            ? item[0].legislation_id
            : item[0].id;
          var last_id = item[0].legislation_id
            ? item[0].legislation_id
            : item[0].container_id
            ? item[0].container_id
            : item[0].id;
          ...
          if (k > 0) {
            ...
            if (onecitation.sortedItems[k - 1][0].legislation_id) {
              ...
              incitationxloc =
                onecitation.sortedItems[k - 1][1]["locator-extra"];
              ...
              for (var l = k - 2; l > -1; l--) {
                if (onecitation.sortedItems[l][1].parallel === "first") {
                  ...
                  incitationxloc =
                    onecitation.sortedItems[l][1]["locator-extra"];
                }
              }
              //}
            }
          ...
          oldvalue["first-container-reference-note-number"] =
            item[1]["first-container-reference-note-number"];
          ...
          if (this.registry.citationreg.citationsByItemId[item_id]) {
            if (this.opt.xclass === "note" && this.opt.has_disambiguate) {
              var oldCount =
                this.registry.registry[item[0].id]["citation-count"];
              var newCount =
                this.registry.citationreg.citationsByItemId[item_id].length;
              this.registry.registry[item[0].id]["citation-count"] =
                this.registry.citationreg.citationsByItemId[item_id].length;
              if ("number" === typeof oldCount) {
                ...
                if (oldCountCheck !== newCountCheck) {
                  for (
                    var l = 0,
                      llen =
                        this.registry.citationreg.citationsByItemId[item_id]
                          .length;
                    l < llen;
                    l++
                  ) {
          ...
          if (
            "undefined" === typeof last_ref[last_id] &&
            ...
            if (j > 0) {
              ...
              if (citations[adjusted_offset].sortedItems.length) {
                oldlastid =
                  citations[adjusted_offset].sortedItems.slice(-1)[0][1].id;
                oldlastxloc =
              ...
              if (prevCitation.sortedItems.length) {
                if (prevCitation.sortedItems[0].slice(-1)[0].legislation_id) {
                  oldlastid =
                    prevCitation.sortedItems[0].slice(-1)[0].legislation_id;
                }
              }
            }
            if (
              ...
              if (
                oldid == first_id &&
                prevCitation.properties.noteIndex >=
                  thisCitation.properties.noteIndex - 1
              ) {
                var prevxloc = prevCitation.sortedItems[0][1]["locator-extra"];
                var thisxloc = thisCitation.sortedItems[0][1]["locator-extra"];
                if (
                  (citationsInNote[prevCitation.properties.noteIndex] === 1 ||
                    prevCitation.properties.noteIndex === 0) &&
                  prevxloc === thisxloc
                ) {
                  useme = true;
                }
              ...
              k > 0 &&
              incitationid == first_id &&
              incitationxloc == myxloc
            ) {
    ...
    if (
      ...
      if (pos === 0) {
        ...
        if (typeof tmpobj === "string") {
          objects[objects.length - 1] += txt_esc(
            this.citation.opt.layout_suffix
          );
    ...
    if (
      ...
      if (this.registry.registry[item.id].parallel_delimiter_override) {
        this.output.queue[0].strings.delimiter =
          this.registry.registry[item.id].parallel_delimiter_override;
      } else {
        this.output.queue[0].strings.delimiter = ", ";
      }
      this.tmp.term_predecessor = false;
      ...
      for (j = 0, jlen = sortedItems.length; j < jlen; j += 1) {
        ...
        entry_item_ids.push(
          "" + CSL.getCite.call(this, sortedItems[j][0], sortedItems[j][1])
        );
        this.tmp.cite_index++;
...
CSL.Engine.prototype.localeSet = function (myxml, lang_in, lang_out) {
  ...
  if (myxml.nodeNameIs(myxml.dataObj, "locale")) {
    ...
    for (pos = 0, len = myxml.numberofnodes(nodes); pos < len; pos += 1) {
      ...
      if (
        !foundLocale &&
        myxml.getAttributeValue(blob, "lang", "xml") === lang_in
      ) {
        locale = blob;
        foundLocale = true;
      } else {
        ...
        if (lang && style_options && style_options.length) {
          var jurispref = myxml.getAttributeValue(
            style_options[0],
          ...
          if (jurispref) {
            ...
            this.locale[lang].opts["jurisdiction-preference"] =
              jurispref.split(/\s+/);
          }
        }
      }
    ...
    if (this.tokentype === CSL.START || this.tokentype === CSL.SINGLETON) {
      ...
      func = function (state, Item) {
        ...
        if (
          ...
          state.opt.disable_duplicate_year_suppression.indexOf(Item.country) ===
            -1 &&
          !state.tmp.extension &&
...
CSL.Node["date-part"] = {
  build: function (state, target) {
    ...
    function formatAndStrip(myform, gender, val) {
      ...
      val =
        "" +
        CSL.Util.Dates[this.strings.name][myform](
          state,
          val,
          gender,
          this.default_locale
        ...
        if (this.realGroup) {
          ...
          var context = {
            ...
            parallel_delimiter_override:
              this.strings.set_parallel_delimiter_override,
            parallel_delimiter_override_on_suppress:
          ...
          if (
            state.tmp.abbrev_trimmer &&
            state.tmp.abbrev_trimmer.LAST_TO_FIRST &&
            context.parallel_last
          ) {
      ...
      if (this.juris) {
        ...
        func = function (state, Item, item) {
          ...
          if (state.juris[itemItem["best-jurisdiction"]][this.juris]) {
            while (
              next <
              state.juris[itemItem["best-jurisdiction"]][this.juris].length
            ) {
              next = CSL.tokenExec.call(
                state,
                state.juris[itemItem["best-jurisdiction"]][this.juris][next],
                Item,
                item
              );
            }
      ...
      func = function (state, Item, item) {
        ...
        if (this.realGroup) {
          ...
          if (flags.parallel_delimiter_override) {
            state.tmp.group_context.tip.parallel_delimiter_override =
              flags.parallel_delimiter_override;
            if (
              !state.tmp.just_looking &&
              state.registry.registry[Item.id].master
            ) {
              state.registry.registry[Item.id].parallel_delimiter_override =
                flags.parallel_delimiter_override;
            }
          }
          ...
          if (state.tmp.area === "bibliography_sort") {
            ...
            if (
              ...
              citationNumberIdx > -1 &&
              state.tmp.group_context.length() == 1
            ) {
              if (this.strings.sort_direction === CSL.DESCENDING) {
                state.bibliography_sort.opt.citation_number_sort_direction =
                  CSL.DESCENDING;
              } else {
                state.bibliography_sort.opt.citation_number_sort_direction =
                  CSL.ASCENDING;
              }
              flags.done_vars = flags.done_vars
                .slice(0, citationNumberIdx)
                .concat(flags.done_vars.slice(citationNumberIdx + 1));
            }
          }
          ...
          if (
            ...
            if (
              ...
              if (
                ...
                if (
                  ...
                  state.output.queue.slice(-1)[0].parallel_delimiter =
                    flags.parallel_delimiter_override_on_suppress;
            ...
            if (flags.force_suppress) {
              ...
              for (var i = 0, ilen = flags.done_vars.length; i < ilen; i++) {
                ...
                for (
                  var j = 0, jlen = state.tmp.done_vars.length;
                  j < jlen;
                  j++
                ) {
                  if (state.tmp.done_vars[j] === doneVar) {
                    state.tmp.done_vars = state.tmp.done_vars
                      .slice(0, j)
                      .concat(state.tmp.done_vars.slice(j + 1));
                  }
                }
              }
            }
          ...
          if (variable === "citation-number") {
            func = function (state, Item) {
              ...
              if (
                ...
                state.bibliography_sort.tmp.citation_number_map
              ) {
                var num =
                  state.bibliography_sort.tmp.citation_number_map[
                    state.registry.registry[Item.id].seq
                  ];
              } else {
        ...
        func = function (state) {
          ...
          if (!state.tmp.parallel_and_not_last) {
            ...
            if (
              ...
              suffix =
                state.tmp.cite_affixes[state.tmp.area][
                  state.tmp.last_cite_locale
                ].suffix;
            } else {
    ...
    if (
      ...
      func = function (state, Item, item) {
        ...
        if (
          ...
          for (var field in state.tmp.abbrev_trimmer.QUASHES[
            Item.jurisdiction
          ]) {
    ...
    if (this.state.tmp.first_name_string) {
      ...
      if (author_title && this.state.sys.getAbbreviation) {
        if (this.state.sys.normalizeAbbrevsKey) {
          author_title = this.state.sys.normalizeAbbrevsKey(
            "classic",
            author_title
          );
        }
...
CSL.NameOutput.prototype._getAfterInvertedName = function (
  blobs,
  delimiter,
  finalJoin
) {
  if (finalJoin && blobs.length > 1) {
    if (
      this.state.inheritOpt(this.name, "delimiter-precedes-last") ===
      "after-inverted-name"
    ) {
...
CSL.NameOutput.prototype._joinEtAl = function (blobs) {
  var delimiter = this.state.inheritOpt(
    this.name,
    "delimiter",
    "name-delimiter",
    ", "
...
CSL.NameOutput.prototype._joinEllipsis = function (blobs) {
  ...
  if (blobs.length > 1) {
    ...
    finalJoin = JSON.parse(
      JSON.stringify(this.name.ellipsis[singleOrMultiple])
    );
    finalJoin = this._getAfterInvertedName(blobs, delimiter, finalJoin);
  }
...
CSL.NameOutput.prototype.fixupInstitution = function (name, varname, listpos) {
  ...
  if (this.state.sys.getAbbreviation) {
    ...
    if (
      ["short", "short-long", "long-short"].indexOf(
      ...
      for (var j = ret["short"].length - 1; j > -1; j--) {
        ...
        if (ret["short"][j].indexOf("|") > -1) {
          ...
          ret["short"] = retShort
            .slice(0, j)
            .concat(splitShort)
            .concat(retShort.slice(j + 1));
        }
      }
    }
    if (
      this.state.opt.development_extensions.legacy_institution_name_ordering
    ) {
      ret["short"].reverse();
    }
    ...
    if (!this.state.tmp.just_looking) {
      if (this.Item.jurisdiction) {
        ...
        if (
          ...
          trimmer[jurisdiction][varname]
        ) {
          for (var i = 0, ilen = ret["short"].length; i < ilen; i++) {
            var frag = ret["short"][i];
            ret["short"][i] = frag
              .replace(trimmer[jurisdiction][varname], "")
              .trim();
          }
        }
      }
...
CSL.NameOutput.prototype._quashChecks = function (jurisdiction, str) {
  ...
  str = lst.pop();
  var date = this.Item["original-date"]
...
CSL.Node.substitute = {
  build: function (state, target) {
    ...
    if (this.tokentype === CSL.START) {
      ...
      if_singleton.test = state.fun.match.any(
        if_singleton,
        state,
        if_singleton.tests
      );
      ...
      if (
        ...
        if (this.variables_real[0] === "citation-number") {
          ...
          func = function (state, Item, item) {
            ...
            if (!state.tmp.just_looking) {
              if (
                state.tmp.area.slice(-5) === "_sort" &&
                this.variables[0] === "citation-number"
              ) {
                if (state.tmp.area === "bibliography_sort") {
...
CSL.Attributes["@position"] = function (state, arg) {
  ...
  var testSubsequentNear = function (Item, item) {
    if (
      item &&
      CSL.POSITION_MAP[item.position] >=
        CSL.POSITION_MAP[CSL.POSITION_SUBSEQUENT] &&
      item["near-note"]
    ) {
      return true;
    }
    ...
    return function (Item, item) {
      ...
      if (item && typeof item.position === "number") {
        if (item.position === 0 && tryposition === 0) {
          ...
          tryposition > 0 &&
          CSL.POSITION_MAP[item.position] >= CSL.POSITION_MAP[tryposition]
        ) {
          return true;
        }
...
CSL.Attributes["@is-multiple"] = function (state, arg) {
  ...
  var func = function (Item) {
    var val = "" + Item[arg];
    var lst = val.split(
      /(?:,\s|\s(?:tot\sen\smet|līdz|oraz|and|bis|έως|και|och|až|do|en|et|in|ir|ja|og|sa|to|un|und|és|și|i|u|y|à|e|a|и|-|–)\s|—|\&)/
    );
    if (lst.length > 1) {
      return true;
    }
...
CSL.Attributes["@track-containers"] = function (state, arg) {
  ...
  state.bibliography.opt.track_container_items =
    state.bibliography.opt.track_container_items.concat(args);
};
...
CSL.Attributes["@parallel-delimiter-override-on-suppress"] = function (
  state,
  arg
) {
  state.opt.parallel.enable = true;
...
CSL.Parallel.prototype.StartCitation = function (sortedItems, out) {
  ...
  for (var i = 0, ilen = sortedItems.length - 1; i < ilen; i++) {
    ...
    if (
      ...
      sortedItems[i][0].seeAlso.length > 0 &&
      !parallelMatchList
    ) {
      freshMatchList = true;
      parallelMatchList = [sortedItems[i][0].id].concat(
        sortedItems[i][0].seeAlso
      );
      ...
      for (var j = 0, jlen = remainder.length; j < jlen; j++) {
        ...
        if (idxEnd) {
          ...
          tempMatchList = tempMatchList
            .slice(0, ididx)
            .concat(tempMatchList.slice(ididx + 1));
        }
      }
    }
    ...
    for (var varname in this.state.opt.track_repeat) {
      if (!currItem[varname] || !nextItem[varname]) {
        ...
        "string" === typeof nextItem[varname] ||
        "number" === typeof nextItem[varname]
      ) {
        // Simple comparison of string values
        if (
          varname === "title" &&
          currItem["title-short"] &&
          nextItem["title-short"]
        ) {
...
CSL.Parallel.prototype.checkRepeats = function (params) {
  ...
  if (this.state.tmp.suppress_repeats) {
    if (
      params.parallel_first &&
      Object.keys(params.parallel_first).length > 0
    ) {
...
CSL.Transform = function (state) {
  ...
  function abbreviate(
    ...
    if (
      ["jurisdiction", "country"].indexOf(family_var) > -1 &&
    ...
    return function (state, Item, item) {
      ...
      if (secondary || tertiary) {
        ...
        CSL.UPDATE_GROUP_CONTEXT_CONDITION(
          ...
          null,
          primary_tok,
          primary_tok.strings.prefix + primary
        );
...
CSL.Util.Names.initializeWith = function (
  ...
  if (
    ...
    (!name
      .replace(/^(?:<[^>]+>)*/, "")
      .match(CSL.STARTSWITH_ROMANESQUE_REGEXP) &&
      !terminator.match("%s"))
  ) {
    return name;
  }
...
CSL.Util.Names.doNormalize = function (state, namelist, terminator) {
  ...
  for (i = 0, ilen = namelist.length; i < ilen; i += 1) {
    if (
      this.notag(namelist[i]).length > 1 &&
      this.notag(namelist[i]).slice(-1) === "."
    ) {
  ...
  if (
    ...
    func = function (state, Item, item) {
      if (!state.tmp.just_looking && !state.tmp.suppress_decorations) {
        ...
        if (item && item["first-container-reference-note-number"]) {
          firstContainerReferenceNoteNumber =
            item["first-container-reference-note-number"];
        }
...
CSL.Engine.prototype.processNumber = function (node, ItemObject, variable) {
  ...
  function parseString(str, defaultLabel) {
    ...
    if (m) {
      ...
      for (var i in lst) {
        if (
          ("" + lst[i]).replace(/^[a-z]\.\s+/, "").match(/[^\s0-9ivxlcmIVXLCM]/)
        ) {
          //recombine = true;
          break;
        }
      ...
      if (m) {
        ...
        if (m.length > 0) {
          ...
          var notAlabel =
            ...
            "undefined" ===
              typeof me.getTerm(CSL.STATUTE_SUBDIV_STRINGS[slug]) ||
            (["locator", "number", "locator-extra", "page"].indexOf(
              variable
            ) === -1 &&
        ...
        for (var j = 0, jlen = lst.length; j < jlen; j++) {
          if (lst[j] || j === lst.length - 1) {
            ...
            if (j === lst.length - 1) {
              ...
              values.push(
                composeNumberInfo(filteredOrigLabel, label, mystr, null, i)
              );
            }
          }
        }
  ...
  function fixNumericAndCount(values, i, currentLabelInfo) {
    ...
    var mCurrentLabel = master.value.match(
      /^(?:[0-9]+|[ixv]+)([-,:a-zA-Z]*|\-[\-0-9]+)$/
    );
  ...
  function fixLabelVisibility(values, groupStartPos, currentLabelInfo) {
    if (currentLabelInfo.label.slice(0, 4) !== "var:") {
      if (currentLabelInfo.pos === 0) {
        if (
          ["locator", "number", "locator-extra", "page"].indexOf(variable) > -1
        ) {
          // Actually, shouldn't we do this always?
          if (
            "undefined" ===
            typeof me.getTerm(
              CSL.STATUTE_SUBDIV_STRINGS[currentLabelInfo.label]
            )
          ) {
        ...
        if (
          ["locator", "number", "locator-extra", "page"].indexOf(variable) ===
          -1
        ) {
  ...
  if (
    ...
    this.tmp.shadow_numbers[realVariable].values.length
  ) {
  ...
  if (val && this.sys.getAbbreviation) {
    ...
    if (this.transform.abbrevs[jurisdiction].number) {
      if (this.transform.abbrevs[jurisdiction].number[normval]) {
        ...
        if (
          "undefined" !==
          typeof this.transform.abbrevs[jurisdiction].number[normval]
        ) {
  ...
  if (
    ...
    if (this.tmp.shadow_numbers[realVariable].values.length === 0) {
      ...
      if (node) {
        ...
        this.tmp.shadow_numbers[realVariable].masterStyling =
          setStyling(values);
...
CSL.Util.outputNumericField = function (state, varname, itemID) {
  ...
  for (var i = 0, ilen = nums.length; i < ilen; i++) {
    ...
    CSL.UPDATE_GROUP_CONTEXT_CONDITION(
      state,
      masterStyling.strings.prefix,
      null,
      masterStyling,
      `${num.particle}${num.value}`
    );
    if (num.collapsible) {
      var blob;
      if (
        num.value.match(/^[1-9][0-9]*$/) &&
        Number.isSafeInteger(parseInt(num.value, 10))
      ) {
...
CSL.Output.Formatters = (function () {
  ...
  function _textcaseEngine(config, string) {
    ...
    if (config.lastWordPos) {
      ...
      if (
        lastWord.length > 1 &&
        CSL.toLocaleLowerCase.call(this, lastWord).match(config.skipWordsRex)
      ) {
...
CSL.Registry.prototype.renumber = function () {
  ...
  //
  if (
    this.state.bibliography_sort.opt.citation_number_sort_direction ===
    CSL.DESCENDING
  ) {
  ...
  for (pos = 0; pos < len; pos += 1) {
    ...
    if (
      ...
      this.state.bibliography_sort.tmp.citation_number_map[item.seq] =
        this.reflist.length - item.seq + 1;
    }
...
CSL.Registry.Comparifier = function (state, keyset) {
  ...
  var sortCompare = CSL.getSortCompare.call(
    state,
    state.opt["default-locale-sort"]
  );
...
CSL.Registry.prototype.registerAmbigToken = function (akey, id, ambig_config) {
  ...
  if (!this.registry[id]) {
    CSL.debug(
      "Warning: unregistered item: itemID=(" + id + "), akey=(" + akey + ")"
    );
  }
  //SNIP-END
...
CSL.Registry.NameReg = function (state) {
  ...
  set_keys = function (state, itemid, nameobj) {
    ...
    if (
      state.opt["demote-non-dropping-particle"] === "never" &&
...
CSL.Disambiguation.prototype.scanItems = function (list) {
  ...
  for (var pos = 1, len = list[1].length; pos < len; pos += 1) {
    ...
    if (this.ItemCite === otherItemCite) {
      ...
      if (this.debug) {
        this.state.sys.print(
          "  [CLASH]--> " + this.Item.id + ": " + this.ItemCite
        );
        this.state.sys.print(
          "             " + otherItem.id + ": " + otherItemCite
        );
      }
      //SNIP-END
      clashes += 1;
      ...
      if (this.debug) {
        this.state.sys.print(
          "  [clear]--> " + this.Item.id + ": " + this.ItemCite
        );
        this.state.sys.print(
          "             " + otherItem.id + ": " + otherItemCite
        );
...
CSL.Disambiguation.prototype.incrementDisambig = function () {
  ...
  if ("disNames" === this.modes[this.modeindex]) {
    ...
    if (this.debug) {
      ...
      this.state.sys.print(
        "    | names value: " + this.base.names[this.gnameset]
      );
...
CSL.Engine.prototype.loadStyleModule = function (
  jurisdiction,
  xmlSource,
  skipFallback
) {
...
// Module export compatibility for different environments
(function () {
  // Node.js module export
  if (typeof module !== "undefined" && typeof module.exports !== "undefined") {
    module.exports = CSL;
  }
  // AMD module export (RequireJS)
  ...
  // Global export for web workers and other environments
  else if (typeof global !== "undefined") {
    global.CSL = CSL;
  }
  // Last resort global export
